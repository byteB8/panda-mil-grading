...
rsync -av --delete -e "ssh -p $SSH_PORT" \
  --exclude data --exclude 'results*' --exclude '*.log' --exclude .git --exclude '__pycache__' \
...
Synced to $DEST. On the server:
  ...
  cd ~/work/mil
  CUDA_VISIBLE_DEVICES=3 nohup ~/wrf/wrfEnv/bin/python -u train.py \
      --features data/pda-ft --out results --quiet --threads 4 > train.log 2>&1 &
  tail -f train.log
MSG
